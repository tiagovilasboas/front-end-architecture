---
title: 'Máquinas de Estado (State Machines)'
description: 'Descubra como usar Máquinas de Estado para gerenciar estados complexos da UI de forma previsível e robusta, eliminando bugs comuns.'
icon: 'workflow'
layout: '../../layouts/ContentLayout.astro'
category: 'Padrões de UI'
---

`isLoading`, `isError`, `isSuccess`, `isEmpty`, `isSubmitting`... Se você já se viu com um monte de variáveis booleanas para controlar o que aparece na tela, você sabe a confusão que isso pode virar. E se `isLoading` e `isError` forem `true` ao mesmo tempo? O que acontece? É a receita para bugs bizarros.
